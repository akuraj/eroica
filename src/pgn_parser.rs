...
pub fn parse_move(mv_str: &str, state: &State) -> Result<Move, String> {
    ...
    match mv_str {
        ...
        _ => {
            ...
            if check_count > 1 {
                return Err(format!("Too many '+'s in {}", mv_str_mut));
            }
            ...
            if is_check && is_checkmate {
                return Err("Input move has both '+' and '#'".to_string());
            }

            // Remove check and checkmate
            mv_str_mut = mv_str_mut
                .chars()
                .filter(|&x| x != '+' && x != '#')
                .collect();
            ...
            if capture_count > 1 {
                return Err(format!("Too many 'x's in the input move: {}", mv_str_mut));
            }
            ...
            if num_poss > 1 {
                ...
                if piece == (state.to_move | PAWN) {
                    ...
                    if size != 1 {
                        return Err(format!(
                            "Disambiguation is problematic 1: {}, {}",
                            mv_str, mv_str_mut
                        ));
                    } else {
                    ...
                    if size > 2 {
                        return Err(format!(
                            "Disambiguation is problematic 2: {}, {}",
                            mv_str, mv_str_mut
                        ));
                    } else if size == 2 {
                        ...
                        let disamb = match mv_str_mut.chars().next() {
                            Some(file_actual) => file_actual,
                            None => {
                                return Err("disamb: mv_str_mut[ 0 ] out of bounds!".to_string())
                            }
                        };
                        ...
                        if ('1'..='8').contains(&disamb) {
                            ...
                            return Err(format!(
                                "Disambiguation is problematic 3: {}, {}",
                                mv_str, mv_str_mut
                            ));
                        }
                    } else {
                        return Err(format!(
                            "Disambiguation is problematic 4: {}, {}",
                            mv_str, mv_str_mut
                        ));
                    }
                }
                ...
                if final_size == 1 {
                    ...
                    Err(format!(
                        "Disambiguation is problematic 5: {}, {}",
                        mv_str, mv_str_mut
                    ))
                }
            } else if num_poss == 1 {
                match possibilities.iter_mut().nth(0) {
                    ...
                    None => {
                        return Err("possibilities: possibilities[ 0 ] out of bounds!".to_string())
                    }
                }
            } else {
